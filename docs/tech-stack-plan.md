...
5. **Store staff scanner + brand console MVP** — split in two, since they're separate products and bundling them made one change too big to verify honestly.
   ...
   - ✅ **5b, brand console:** `/brand` — a brand member creates a campaign (a reward pool funded on one game for a date range) and gets a dashboard of plays, minutes played, new players, store visits, pool burn, cost per play and cost per visit, plus a plays-per-day chart (Recharts). Every figure is real SQL over `play_sessions` and `voucher_redemptions`; nothing is estimated. Funding is an admin-confirmed action on `/admin`, not Stripe, and `campaignStatus()` reports "draft" until it happens, so an unfunded campaign shows no numbers at all. New `brands`, `brand_members` and `campaigns` tables; `rewards.brand_name`/`stores.brand_name` replaced by `brand_id`, which also retires the staff scanner's string match (the 5a carry-over). Money is `campaigns.budget_fils`, integer minor units — the only currency in the schema.
     - **Dropped from the brief on purpose:** city targeting. `stores.city` is the only geo column anywhere — there's nothing on profiles, plays or games — so a city selector couldn't affect who sees or plays a game, only look as though it did. Needs player-side geo before it means anything.
     - **Still deferred:** the funnel, live activity feed and ROI card (near-empty at current traffic), Realtime (no Supabase client is installed; the dashboard is fresh-on-load), and the forecast (phase 7, below).
6. **Admin:** ✅ moderation (pulled forward ahead of phase 5, since nothing could drain the queue phase 4 started filling — `/admin`, gated by an `ADMIN_EMAILS` allowlist rather than a database role; approve, or reject with a reason the creator sees). ✅ **fraud review** — `/admin/fraud` groups accounts by rejected plays, referrals and voucher volume; rejections now store the claimed score so it can be shown against the template's real ceiling; an admin can suspend an account, which blocks earning and spending but not reading, and leaves credited points alone. ✅ **rewards/pools management** — `/admin/rewards` creates, edits, tops up and deactivates; top-ups move total and remaining together SQL-side. Still to do: **platform analytics** (DAU, retention, K-factor, redemption rate are all computable from existing timestamps — deferred until there's enough traffic that a chart can be told apart from a flat line), Admin actions are now audited: every action under `/admin` writes an `admin_actions` row readable at `/admin/activity`, which also closed the gap where campaign funding and suspensions recorded the outcome but not the actor. Still open on the fraud side: no IP/device/login-event data, so multi-accounting isn't detectable — a login log is the obvious fix but needs a data-retention and privacy decision first.
