...
import { THEMES, artSVG, icon, itemShape, type GameArtType, type ItemKind, type ThemeName } from "@playloop/ui";
...
const TEMPLATES: { type: PlayableType; name: string; blurb: string; best: string }[] = [
  { type: "quiz", name: "Quiz Blitz", blurb: "Four answers, one right. Speed earns a bonus.", best: "Trivia, launches, learning" },
  { type: "memory", name: "Memory Match", blurb: "Flip and match six pairs. Use your own images.", best: "Products, heritage, art" },
  { type: "catch", name: "Catch and Collect", blurb: "Drag to catch falling items. Dodge the bad ones.", best: "Hero products, campaigns" },
  { type: "reflex", name: "Tap Reflex", blurb: "Tap the good, dodge the bad. Chain combos.", best: "Events and big screens" },
];
...
export function CreatorWizard({ aiProviderLabel }: { aiProviderLabel: string }) {
  ...
  const issues = useMemo(() => (draft.type ? validateGameDraft(gameDraft) : []), [gameDraft, draft.type]);
  const issueFor = (field: string) => (showErrors ? issues.find((i) => i.field === field)?.message : undefined);
  ...
  async function publish() {
    ...
    setError(null);
    try {
      const game = await publishGame(gameDraft);
      router.push(`/create/games/${game.id}?published=1`);
    } catch (e) {
      setError(e instanceof Error ? e.message : "Couldn't publish that game — try again.");
      setPublishing(false);
    }
  }

  if (testing) return <TestPlay draft={gameDraft} onClose={() => setTesting(false)} />;
  ...
  return (
    <main className="mx-auto max-w-md p-6">
      ...
      {step === 2 && draft.type ? (
        <section>
          <h2 className="text-xl font-extrabold">Test it before it goes live</h2>
          <p className="mt-1 mb-4 text-sm font-bold text-soft">
            Play it exactly as players will. Nothing is published, and this test earns no points.
          </p>
          <button className="btn go block" onClick={() => setTesting(true)}>
            <span dangerouslySetInnerHTML={{ __html: icon("play", "fill") }} /> Test play
          </button>
        </section>
      ) : null}
      ...
      {step === 3 && draft.type ? (
        <section>
          ...
          <button className="btn go block" onClick={publish} disabled={publishing}>
            {publishing ? (
              <>
      ...
      <div className="mt-8 flex gap-3">
        {step > 0 ? (
          <button className="btn" onClick={() => setStep((s) => s - 1)} disabled={publishing}>
            Back
          </button>
        ) : null}
        {step < STEPS.length - 1 ? (
          <button className="btn go flex-1" onClick={next} disabled={step === 0 && !draft.type}>
...
function Field({ label, hint, error, children }: { label: string; hint?: string; error?: string; children: React.ReactNode }) {
  return (
    <div className="mb-5">
      ...
      {hint ? <p className="mb-2 text-xs font-bold text-soft">{hint}</p> : <div className="mb-2" />}
      {children}
...
const inputClass = "w-full rounded-2xl bg-card p-3 font-semibold [border:var(--border-thick)]";
...
function Customise({
  ...
  return (
    <section>
      ...
      <p className="mt-1 mb-4 text-sm font-bold text-soft">The cover above updates as you go.</p>
      ...
      <Field label={`Max points per play — ${draft.maxPoints}`} error={issueFor("maxPoints")}>
        <input
          type="range"
          className="w-full"
          min={MAX_POINTS_MIN}
      ...
      {draft.type === "memory" ? (
        ...
        >
              ...
              >
                {im ? (
                  <>
                    ...
                    <img src={im} alt={`Pair ${i + 1}`} className="h-full w-full object-cover" />
                    <button
          ...
          <label className="btn sm mt-3 inline-flex">
            <span dangerouslySetInnerHTML={{ __html: icon("upload") }} /> Upload images
            <input
              type="file"
              accept="image/*"
              multiple
...
function QuizEditor({
  ...
  const update = (i: number, patch: Partial<Draft["questions"][number]>) => {
    const questions = draft.questions.map((q, k) => (k === i ? { ...q, ...patch } : q));
    set({ questions });
  };
    ...
    <Field label="Questions" hint="Tap a letter to mark the right answer." error={issueFor("questions")}>
            ...
            <div className="mb-2 flex items-center justify-between">
              ...
              {draft.questions.length > 1 ? (
                <button
                  ...
                  onClick={() => set({ questions: draft.questions.filter((_, k) => k !== i) })}
                >
                  Delete
                </button>
              ) : null}
              ...
              {q.a.map((a, k) => (
                <div key={k} className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    value={a}
                    onChange={(e) => update(i, { a: q.a.map((v, j) => (j === k ? e.target.value : v)) })}
                    suppressHydrationWarning
                  />
                </div>
              ))}
      ...
      {draft.questions.length < QUIZ_MAX_QUESTIONS ? (
        <button
          ...
          onClick={() => set({ questions: [...draft.questions, { q: "", a: ["", "", "", ""], c: 0 }] })}
        >
          <span dangerouslySetInnerHTML={{ __html: icon("plus") }} /> Add question
        </button>
      ) : null}
    </Field>
  );
