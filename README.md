...
**Status (this commit): earn-and-spend loop works end to end, with a server-verified play session** — email login → onboarding → feed → play (Catch, Quiz, Memory or Reflex, server-issued and server-validated) → points/XP → spend points in Rewards on a real, scannable-QR voucher → see it all in Wallet. Challenges, the creator studio, and the brand console are later phases (see `docs/tech-stack-plan.md`).
